...
a.active_votes.forEach(function(b){0>b.percent?a.down_votes.push(b):0<b.percent&&a.up_votes.push(b)});var c=Session.getAlive();a.reported=!1;if(c){c=c.account.username;for(var d=0;d<a.down_votes.length;d++)if(a.down_votes[d].voter===c){a.reported=!0;break}}c=function(a){return"object"===typeof a?Asset.parse(a).toFriendlyString(null,!1):a};a.refused_payouts=0>=Asset.parse(a.max_accepted_payout).amount;a.curator_payout_value=c(a.curator_payout_value);a.max_accepted_payout=c(a.max_accepted_payout);a.pending_payout_value=
c(a.pending_payout_value);a.promoted=c(a.promoted);a.total_payout_value=c(a.total_payout_value);a.total_pending_value=c(a.total_pending_value);a.reblogged_by&&0!==a.reblogged_by.length||(a.reblogged_by=b)}return a}function getAccounts(a,b){crea.api.getAccounts(a,function(a,d){if(b)if(a)b(a);else{for(a=0;a<d.length;a++)d[a]=parseAccount(d[a]);b(null,d)}})}
