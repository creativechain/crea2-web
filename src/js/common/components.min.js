...
new Date,b=toLocaleDate(this.$props.post.cashout_time);return a.getTime()>b.getTime()},getVote:function(){var a=this.$props.session,b=this.$props.post;if(a&&b){b=b.up_votes;for(var c=0;c<b.length;c++){var d=b[c];if(a.account.username===d.voter)return d}}return null},removeVote:function(a){var b=this.post;if(b){b=b.up_votes;for(var c=-1,d=0;d<b.length;d++)if(a===b[d].voter){c=d;break}console.log(c,a);-1<c&&(this.post.up_votes.slice(c,1),this.$forceUpdate())}},hasVote:function(){return null!=this.getVote()},
makeVote:function(a){a&&a.preventDefault();if(this.state!==this.states.LIKE_OP){var b=this;a=this.$props.session;var c=this.$props.post;a=a?a.account.username:null;var d=b.hasVote()?0:1E4;requireRoleKey(a,"posting",function(a,e){b.state=b.states.LIKE_OP;crea.broadcast.vote(a,e,c.author,c.permlink,d,function(a,f){console.log(a,f);a?b.$emit("vote",a,null,c):(0<d?(b.post.up_votes.push({voter:e,author:c.author,permlink:c.permlink,percent:d}),b.state=b.states.LIKED_END):(b.removeVote(e),b.state=b.states.NO_LIKE_END),
b.$emit("vote",null,f,c))})})}}},mounted:function(){this.state=this.hasVote()?LIKE_STATE.LIKED:LIKE_STATE.NO_LIKE},updated:function(){this.state!=LIKE_STATE.LIKE_OP&&(this.state=this.hasVote()?LIKE_STATE.LIKED:LIKE_STATE.NO_LIKE)}});
...
b.states.NO_LIKE_END),b.$emit("vote",null,f,c));b.$forceUpdate();console.log(a,b.state)})})}}},mounted:function(){this.state=this.hasVote()?LIKE_STATE.LIKED:LIKE_STATE.NO_LIKE},updated:function(){this.state!==LIKE_STATE.LIKE_OP&&(this.state=this.hasVote()?LIKE_STATE.LIKED:LIKE_STATE.NO_LIKE)}});
...
Vue.component("btn-follow",{template:'<div v-on:click="performFollow" v-on:mouseleave="onleave" v-on:mouseover="onover" class="btn btn-sm running ld ld-ext-right font-weight-bold" v-bind:class="btnClasses">\n<div class="btn__text ld-spin-fast ld" v-bind:class="textClasses"></div>{{ text }}<div></div>\n</div>',props:{session:{type:[Object,Boolean]},account:{type:[Object,Boolean]},user:{type:String}},watch:{user:{immediate:!0,deep:!0,handler:function(a,b){this.$forceUpdate()}},account:{immediate:!0,
deep:!0,handler:function(a,b){this.$forceUpdate()}}},computed:{btnClasses:function(){return{"btn--primary":this.state===this.states.NO_FOLLOWING||this.state===this.states.UNFOLLOWED||this.state===this.states.FOLLOWING_OP,"btn-following":!this.over&&(this.state===this.states.FOLLOWING||this.state===this.states.FOLLOWED)||this.state===this.states.UNFOLLOWING_OP,"btn-unfollow":this.over&&(this.state===this.states.FOLLOWING||this.state===this.states.FOLLOWED)}},textClasses:function(){return{text__dark:!this.over&&
(this.state===this.states.FOLLOWING||this.state===this.states.FOLLOWED),"ld-ring":this.state===this.states.FOLLOWING_OP||this.state===this.states.UNFOLLOWING_OP,"ld-ring-blue":this.state===this.states.UNFOLLOWING_OP}}},data:function(){return{lang:lang,over:!1,states:FOLLOW_STATE,state:FOLLOW_STATE.NO_FOLLOWING,lastState:FOLLOW_STATE.NO_FOLLOWING,text:null}},methods:{isStateOp:function(){return this.state===this.states.FOLLOWING_OP||this.state===this.states.UNFOLLOWING_OP},isStateFollowing:function(a){return a===
this.states.FOLLOWING||a===this.states.FOLLOWED},performFollow:function(){if(!this.isStateOp()){var a=this,b=this.$props.session,c=this.state;this.state=this.isStateFollowing(this.state)?this.states.UNFOLLOWING_OP:this.states.FOLLOWING_OP;if(b){var d={follower:b.account.username,following:this.$props.user,what:this.isStateFollowing(c)?[]:["blog"]};d=["follow",d];requireRoleKey(b.account.username,"posting",function(f){crea.broadcast.customJson(f,[],[b.account.username],"follow",jsonstring(d),function(b,
d){b?(a.state=c,a.$emit("follow",b)):(a.state=a.isStateFollowing(c)?a.states.UNFOLLOWED:a.states.FOLLOWED,a.$emit("follow",null,d))})})}else this.state=c,this.$emit("follow",Errors.USER_NOT_LOGGED)}},onover:function(){this.over=!0},onleave:function(){this.over=!1},isFollowing:function(){return this.session&&this.account.followings.includes(this.user)},updateText:function(){switch(this.state){case this.states.NO_FOLLOWING:case this.states.UNFOLLOWED:this.text=this.lang.BUTTON.FOLLOW;break;case this.states.FOLLOWING:case this.states.FOLLOWED:this.text=
this.over?this.lang.BUTTON.UNFOLLOW:this.lang.BUTTON.FOLLOWING;break;case this.states.FOLLOWING_OP:this.text=this.text=this.lang.BUTTON.FOLLOW;break;case this.states.UNFOLLOWING_OP:this.text=this.lang.BUTTON.FOLLOWING}}},updated:function(){this.isStateOp()||(this.state=this.isFollowing()?this.states.FOLLOWING:this.states.NO_FOLLOWING);this.updateText()},mounted:function(){this.state=this.isFollowing()?this.states.FOLLOWING:this.states.NO_FOLLOWING;this.updateText()}});
Vue.component("username",{template:'<a v-bind:href="\'/@\' + user" class="color-name"><p>{{ name || user }}</p></a>',props:{user:{type:String},name:{type:String},inline:{type:Number,"default":1}}});Vue.component("linkname",{template:"<a v-bind:href=\"'/@' + user\" class=\"link-username\">{{ name || '@' + user }}</a>",props:{user:{type:String},name:{type:String}}});
Vue.component("avatar",{template:"<div class=\"img-user-avatar\" v-bind:style=\"{ 'background-image': 'url(' + ( getDefaultAvatar(account)) + ')' }\"></div>",props:{account:{type:Object}},methods:{getDefaultAvatar:R.getAvatar}});
