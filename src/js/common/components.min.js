...
placeholder:String},data:function(){return{value:"",selectedItem:null,matchedItems:[]}},methods:{onFocus:function(){this.matchedItems=this.items},onBlur:function(){var a=this;this.$emit("item",this.$refs.countryInput.value);setTimeout(function(){a.matchedItems=[]},200)},onInput:function(a){a=a.target.value.toLowerCase();for(var b=[],c=0;c<this.items.length;c++){var d=this.items[c];d.name.toLowerCase().startsWith(a)&&(d.textMatched=d.name.substr(0,a.length),d.textUnmatched=d.name.substr(a.length,d.name.length),
b.push(d))}this.matchedItems=b},selectItem:function(a){this.selectedItem=a;this.$refs.countryInput.value=a.name+" (+"+a.callingCodes[0]+")";this.matchedItems=[];this.$emit("item",a)}}});
Vue.component("recommend-post",{template:'<ul class="ul-recommended-post">   <li>       <a href="#0" class="d-flex" v-on:click="makeRecommend">           \x3c!-- Para actilet el parpadeo cuando alguien hace click en el icono, poner esta clases: pre-recommended --\x3e           \x3c!-- Hay dos estados: icon-recommended-post (por defecto), icon-recommended-post-active cuando esta recomendado --\x3e           <div v-bind:class="recommendedClasses" v-on:mouseover="onHover(true)" v-on:mouseleave="onHover(false)"></div>           <span>({{ countRecommended() }})</span>       </a>   </li></ul>',props:{post:Object,
...
b.$emit("vote",null,f,c));console.log(a,b.state,b.$refs.heart_like);b.$refs.heart_like.blur();b.$forceUpdate()})})}}},mounted:function(){this.state=this.hasVote()?LIKE_STATE.LIKED:LIKE_STATE.NO_LIKE},updated:function(){console.log("Updating card like",this.state,this.hasVote());this.state!==LIKE_STATE.LIKE_OP&&(this.state=this.hasVote()?LIKE_STATE.LIKED:LIKE_STATE.NO_LIKE)}});
...
Vue.component("btn-follow",{template:'<div v-on:click="performFollow" v-on:mouseleave="onleave" v-on:mouseover="onover" class="btn btn-sm running ld ld-ext-right font-weight-bold" v-bind:class="btnClasses">   \n<div class="btn__text" v-bind:class="textClasses"></div>   {{ text }}   <div v-if="state === states.FOLLOWING_OP || state === states.UNFOLLOWING_OP" class="loading-button">       <div class="">           <svg viewBox="0 0 50 50" class="spinner">               <circle cx="25" cy="25" r="22.5" class="ring"></circle>               <circle cx="25" cy="25" r="22.5" class="line"></circle>           </svg>       </div>   </div>\n</div>',props:{session:{type:[Object,
