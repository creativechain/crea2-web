...
parseAccount(h.accounts[a])}),c.post=parsePost(h.content[l],b),c.author=parseAccount(h.accounts[c.post.author]),b=Object.keys(h.content),b.sort(function(a,b){a=new Date(h.content[a].created);return(new Date(h.content[b].created)).getTime()-a.getTime()}),b.forEach(function(a){c.post[a]=parsePost(h.content[a])}),c.post.comments=b,a(c),setTimeout(function(){g(e.author,e.permlink,c)},300))};d.when("done",function(a){a=jsonify(a).data;f(a.reblogged_by)});d.when("fail",function(a,b,c){console.error(b,c);
f()});d.headers={Authorization:"Bearer "+b};d.get({})}))})}var k,w,t,u,v,q,h,f;$(window).bind("popstate",function(d){d.originalEvent.state&&d.originalEvent.state.post&&a(d.originalEvent.state,!0)});creaEvents.on("navigation.state.update",function(a){console.log("updating navigation state");if(k){var c=k.state;a.post=c.post;a.postIndex=a.discussions.indexOf(c.post.link);a.author=c.author;a.discuss=c.discuss;a.category=c.category;k.state=a;k.$forceUpdate()}});creaEvents.on("navigation.post.data",p);
