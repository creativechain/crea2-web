...
dateFromNow:function(b){return moment(toLocaleDate(b)).fromNow()},formatDate:function(b){return moment(b+"Z").format("LLLL")},hasPaid:function(b){b=b||this.state.post;var a=new Date;b=toLocaleDate(b.cashout_time);return a.getTime()>b.getTime()},getPayoutPostDate:function(b){b=b||this.state.post;var a=toLocaleDate(b.cashout_time);this.hasPaid(b)&&(a=toLocaleDate(b.last_payout));return moment(a).fromNow()},hasPromotion:function(){return 0<Asset.parseString(this.state.post.promoted).amount},getPromotion:function(){return"$ "+
