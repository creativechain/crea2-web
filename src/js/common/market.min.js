...
b,c,d){return"buy"===c.type.toLowerCase()?'<span class="color-buy">'+a+"</span>":'<span class="color-sell">'+a+"</span>"}},{data:"crea",render:$.fn.dataTable.render.ellipsis()},{data:"cbd",render:$.fn.dataTable.render.ellipsis()}]}))}function B(a,b){g=a;u=b;C()}var e,A,q,w,l,r,v,t,f,g=null,u=null,h=new RpcWsClient("wss://nodes.creary.net");h.connect();h.on("ws.message",function(a,b){console.log("WS MSG",a,b)});creaEvents.on("crea.session.login",function(a,b){creaEvents.emit("crea.dom.ready");B(a,
b);setTimeout(function(){console.log("session started");K();J(3E3)},500)});creaEvents.on("crea.session.update",function(a,b){B(a,b)});$(window).on("beforeunload",function(){console.log("before unload event");h.close()})})();
