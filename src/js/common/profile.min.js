...
catchError(a)||Vue.set(b.profile,"avatar",c)})})}},loadAvatar:function(a){cancelEventPropagation(a);$("#profile-edit-input-avatar").click()},suggestPassword:function(){this.changePass.newPass="P"+crea.formatter.createSuggestedPassword()},claimRewards:K,sendAccountUpdate:E,ignoreUser:function(){ignoreUser(this.state.user.name,!0,function(a,b){updateUserSession()})},changePassword:function(){var a=this,b=function(b){a.changePass.error=b;globalLoading.show=!1};this.changePass.oldPass?this.changePass.newPass&&
this.changePass.newPass===this.changePass.matchedPass?this.changePass.checkedLostPass&&this.changePass.checkedStoredPass?Session.create(this.changePass.username,this.changePass.oldPass).login(function(c,d){c?c===Errors.USER_LOGIN_ERROR&&b(a.lang.CHANGE_PASSWORD.ERROR_CURRENT_PASSWORD):(c=Account.generate(a.changePass.username,a.changePass.newPass,DEFAULT_ROLES).keys,E(null,c,function(a,b){if(a=Session.getAlive())a.logout(),goTo("/")}))}):b(a.lang.CHANGE_PASSWORD.ERROR_CONDITIONS):b(a.lang.CHANGE_PASSWORD.ERROR_MATCHED_PASSWORDS):
b(a.lang.CHANGE_PASSWORD.ERROR_CURRENT_PASSWORD)}}});l.$forceUpdate();creaEvents.emit("crea.dom.ready")}function m(a,b,c){var f=a.replace("s","").replace("-","_"),d={rewards24Vests:0,rewardsWeekVests:0,totalRewardsVests:0,rewards24Crea:0,rewardsWeekCrea:0,totalRewardsCrea:0,rewards24CBD:0,rewardsWeekCBD:0,totalRewardsCBD:0},h=moment(),D=h.subtract(1,"day").valueOf(),e=h.subtract(7,"days").valueOf(),q,k,g=[];c.user.transfer_history.sort(function(a,b){a=toLocaleDate(a[1].timestamp);return toLocaleDate(b[1].timestamp).valueOf()-
a.valueOf()});c.user.transfer_history.forEach(function(a){if("author_reward"===a[1].op[0]&&"author_reward"===f){k||(k=toLocaleDate(a[1].timestamp).valueOf());q=toLocaleDate(a[1].timestamp).valueOf();var b=Asset.parseString(a[1].op[1].vesting_payout),c=Asset.parseString(a[1].op[1].crea_payout),h=Asset.parseString(a[1].op[1].cbd_payout);q>e&&(q>D&&(d.rewards24Vests+=b.toFloat(),d.rewards24Crea+=c.toFloat(),d.rewards24CBD+=h.toFloat()),d.rewardsWeekVests+=b.toFloat(),d.rewardsWeekCrea+=c.toFloat(),d.rewardsWeekCBD+=
h.toFloat());d.totalRewardsVests+=b.toFloat();d.totalRewardsCrea+=c.toFloat();d.totalRewardsCBD+=h.toFloat();g.push(a)}else"curation_reward"===a[1].op[0]&&"curation_reward"===f&&(k||(k=toLocaleDate(a[1].timestamp).valueOf()),q=toLocaleDate(a[1].timestamp).valueOf(),b=Asset.parseString(a[1].op[1].reward),q>e&&(q>D&&(d.rewards24Vests+=b.toFloat()),d.rewardsWeekVests+=b.toFloat()),d.totalRewardsVests+=b.toFloat(),g.push(a))});v[a]?(v[a].session=b,v[a].state=c,v[a].rewards=d,v[a].rewardsOp=g):v[a]=new Vue({el:"#profile-"+
a,data:{lang:lang,session:b,state:c,rewards:d,rewardsOp:g},methods:{vestsToCgy:function(a){function b(b){return a.apply(this,arguments)}b.toString=function(){return a.toString()};return b}(function(a){return vestsToCgy(this.state,a)}),parseAsset:Asset.parse,formatTime:function(a){return moment(a).format("DD MMM HH:MM")}}})}function n(a,b,c){z?(z.session=a,z.account=b,z.blocked=c):z=new Vue({el:"#blocked-container",data:{lang:lang,session:a,account:b,blocked:c},methods:{unlock:function(a){ignoreUser(a,
!1,function(a,b){catchError(a)||updateUserSession()})}}})}function r(a,b,c,p){w?(w.state=a,w.session=b,w.account=c,w.following=p):w=new Vue({el:"#following-container",data:{lang:lang,state:a,session:b,account:c,following:p},methods:{onFollow:function(){updateUserSession()}}})}function y(a,b,c,p){x?(x.state=a,x.session=b,x.account=c,x.follower=p):x=new Vue({el:"#follower-container",data:{lang:lang,state:a,session:b,account:c,follower:p},methods:{onFollow:function(){updateUserSession()}}})}function L(a){var b=
getPathPart().replace("@","");F(b,"transfers",function(b,f){catchError(b)||(m("author-rewards",a,f),m("curation-rewards",a,f))})}function M(a,b){a&&crea.api.getFollowing(a.account.username,"","ignore",1E3,function(c,f){if(!catchError(c)){var d=[];f=f.following;f.forEach(function(b){b.follower===a.account.username&&(d.includes(b.following)||d.push(b.following))});d.length?crea.api.getAccounts(d,function(c,d){if(!catchError(c)){c={};for(var h=0;h<d.length;h++){var f=d[h];f.metadata=jsonify(f.json_metadata);
c[f.name]=f}n(a,b,c)}}):(console.log("Not blockeds"),n(a,b,{}))}})}function N(a,b,c){var f=function(d){crea.api.getAccounts(d.user.followings,function(d,f){if(!catchError(d)){var h={};f.forEach(function(a){a.metadata=jsonify(a.json_metadata);a.metadata.avatar=a.metadata.avatar||{};h[a.name]=a});r(a,b,c,h)}})};c&&a.user.name===c.user.name?f(c):crea.api.getFollowing(a.user.name,"","blog",1E3,function(b,c){if(!catchError(b)){var d=[];c.following.forEach(function(a){d.push(a.following)});a.user.followings=
d;f(a)}})}function O(a,b,c){var f=function(d){crea.api.getAccounts(d.user.followers,function(d,f){if(!catchError(d)){var h={};f.forEach(function(a){a.metadata=jsonify(a.json_metadata);a.metadata.avatar=a.metadata.avatar||{};h[a.name]=a});y(a,b,c,h)}})};crea.api.getFollowers(a.user.name,"","blog",1E3,function(b,c){if(!catchError(b)){var d=[];c.followers.forEach(function(a){d.push(a.follower)});a.user.followers=d;f(a)}})}function P(a,f,c,p){var d=getPathPart(null,1),h="balances";if(!d||d.isEmpty())d=
"projects";switch(d){case "balances":case "passwords":case "permissions":h=d,d="wallet"}d=d.toLowerCase();e(a,f,c,p,d,h);f&&f.account.username===a.user.name&&(g(a,f),b(a,f));Q(a.user.name);L(f);M(f,c);N(a,f,c);O(a,f,c)}function E(a,b,c){var f=toLocaleDate(l.state.user.last_account_update);if(moment().valueOf()-f.valueOf()>=CONSTANTS.ACCOUNT.UPDATE_THRESHOLD){var d=Session.getAlive();if(d){var h=l.profile;h.tags=$("#profile-edit-tags").val().split(" ");h=jsonstring(h);b||(a=l.state.user,b={memo:{pub:a.memo_key},
active:{pub:a.active.key_auths[0][0]},posting:{pub:a.posting.key_auths[0][0]},owner:{pub:a.owner.key_auths[0][0]}});console.log(b);requireRoleKey(d.account.username,"owner",function(a){globalLoading.show=!0;crea.broadcast.accountUpdate(a,d.account.username,createAuth(b.owner.pub),createAuth(b.active.pub),createAuth(b.posting.pub),b.memo.pub,h,function(a,b){globalLoading.show=!1;a?c&&c(a):(updateUserSession(),c&&c(null,b))})})}else globalLoading.show=!1,c&&c(Errors.USER_NOT_LOGGED)}else a=lang.ERROR.ACCOUNT_UPDATE_THRESHOLD_EXCEEDED.TITLE,
f=String.format(lang.ERROR.ACCOUNT_UPDATE_THRESHOLD_EXCEEDED.BODY,toLocaleDate(f).fromNow()),showAlert(a,f)}function Q(a){setTimeout(function(){crea.api.getAccountHistory(a,-1,50,function(a,b){if(!catchError(a)){b.history=b.history.reverse();var c=[],d=[];b.history.forEach(function(a){a=a[1];var b=function(a){a&&0>c.indexOf(a)&&c.push(a)};"transfer_operation"===a.op.type?(b(a.op.value.from),b(a.op.value.to)):"transfer_to_vesting_operation"===a.op.type?(b(a.op.value.from),b(a.op.value.to)):"transfer_to_savings_operation"===
a.op.type?(b(a.op.value.from),b(a.op.value.to)):"vote_operation"===a.op.type?(b(a.op.value.voter),b(a.op.value.author)):"comment_operation"===a.op.type?(b(a.op.value.parent_author),b(a.op.value.author)):"producer_reward_operation"===a.op.type?b(a.op.value.producer):"account_create_operation"===a.op.type?b(a.op.value.creator):"curation_reward_operation"===a.op.type?(b(a.op.value.curator),b(a.op.value.comment_author)):"comment_download_operation"===a.op.type&&(b(a.op.value.downloader),b(a.op.value.comment_author));
d.push(a)});crea.api.getAccounts(c,function(a,b){if(a)console.error(a);else{var f={};c.forEach(function(a){for(var c=0;c<b.length;c++)if(a===b[c].name){f[a]=parseAccount(b[c]);break}});l.history.data=d;l.history.accounts=f}})}})})}function F(a){var b=1<arguments.length&&void 0!==arguments[1]?arguments[1]:null,c=2<arguments.length&&void 0!==arguments[2]?arguments[2]:null;var e=a.startsWith("/@")?a:"/@"+a;b&&("string"===typeof b?e=e+"/"+b:"function"===typeof b&&(c=b));crea.api.getState(e,function(b,
f){b?c&&c(b):(f.accounts[a]&&(f.user=parseAccount(f.accounts[a])),f.content={},f.discussion_idx={"":{profile:[]}},c&&c(b,f))})}function S(a,b){var c=getPathPart();(c=c.replace("@",""))&&F(c,function(f,d){catchError(f)||(console.log(clone(d)),G(function(f,e){Object.keys(d.accounts).forEach(function(a){d.accounts[a]=parseAccount(d.accounts[a])});d.accounts[c]&&(d.user=d.accounts[c],crea.formatter.estimateAccountValue(d.user).then(function(a){d.user.estimate_account_value=a}));e.sort(function(a,b){a=
toLocaleDate(a.created);return toLocaleDate(b.created).valueOf()-a.valueOf()});for(f=0;f<e.length;f++){var h=e[f],g=h.author+"/"+h.permlink;d.content[g]=h;d.discussion_idx[""].profile.push(g)}P(d,a,b,c)}))})}function K(a){a&&a.preventDefault();var b=l.state.user.reward_crea_balance,c=l.state.user.reward_cbd_balance,e=l.state.user.reward_vesting_balance;requireRoleKey(l.session.account.username,"posting",function(a){globalLoading.show=!0;crea.broadcast.claimRewardBalance(a,l.session.account.username,
b,c,e,function(a,b){globalLoading.show=!1;catchError(a)||updateUserSession()})})}function J(a,b,c,e,d,h,g){"function"===typeof h&&(g=h,h="");if(c)switch(c=c.account.username,b){case CONSTANTS.TRANSFER.TRANSFER_CREA:case CONSTANTS.TRANSFER.TRANSFER_CBD:crea.broadcast.transfer(a,c,e,d,h,g);break;case CONSTANTS.TRANSFER.TRANSFER_TO_SAVINGS_CREA:case CONSTANTS.TRANSFER.TRANSFER_TO_SAVINGS_CBD:crea.broadcast.transferToSavings(a,c,e,d,h,g);break;case CONSTANTS.TRANSFER.TRANSFER_FROM_SAVINGS_CREA:case CONSTANTS.TRANSFER.TRANSFER_FROM_SAVINGS_CBD:crea.broadcast.transferFromSavings(a,
c,parseInt(moment().valueOf()/1E3),e,d,h,g);break;case CONSTANTS.TRANSFER.TRANSFER_TO_VESTS:crea.broadcast.transferToVesting(a,c,e,d,g)}else g&&g(Errors.USER_NOT_LOGGED)}function H(a,b){var c=getPathPart(null,1);c="settings"===c.toLowerCase();if(a){if(c&&(c=getPathPart(),"@"+a.account.username!==c)){showProfile(c);return}b.user.cgy_balance="0.000 "+apiOptions.symbol.CGY;a.account.username===b.user.name&&(B={op:"transfer_crea",title:lang.WALLET.TRANSFER_CREA_TITLE,text:lang.WALLET.TRANSFER_CREA_TEXT,
button:lang.BUTTON.SEND,total_amount:Asset.parseString("0.000 CREA").toFriendlyString(),nai:apiOptions.symbol.CREA,confirmed:!1,to:null,disableTo:!1})}else if(c){a=getPathPart();showProfile(a);return}S(a,b)}function G(a){u||(u=1);var b=new HttpClient(apiOptions.apiUrl+"/creary/blog");b.when("done",function(b){b=jsonify(b).data;var c=[],d=b.length,f=function(){--d;0>=d&&a&&a(null,c)};b.forEach(function(a){var b=a.author+"/"+a.permlink;crea.api.getContent(a.author,a.permlink,function(d,e){d||0>=e.author.length?
console.error("Error getting",b,d):(d=parsePost(e),d.reblogged_by=a.reblogged_by,c.push(d));f()})});0===b.length?f():++u});b.when("fail",function(a,b,d){C=!1;catchError(b)});refreshAccessToken(function(a){b.headers={Authorization:"Bearer "+a};b.post({author:getPathPart().replace("@",""),page:u})})}var l,v={},z,w,x,t,A,B,u=1;creaEvents.on("crea.session.login",H);creaEvents.on("crea.session.update",function(a,b){--u;H(a,b)});var C;creaEvents.on("crea.scroll.bottom",function(){C||(C=!0,G(function(a,
b){b.sort(function(a,b){a=toLocaleDate(a.created);return toLocaleDate(b.created).valueOf()-a.valueOf()});for(a=0;a<b.length;a++){var c=b[a],e=c.author+"/"+c.permlink;l.state.content[e]=c;l.state.discussion_idx[""].profile.push(e)}l.state.discussions=l.state.discussion_idx[""].profile;l.$forceUpdate();C=!1;creaEvents.emit("navigation.state.update",l.state)}))})})();
