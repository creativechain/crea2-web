...
b){a=Asset.parse(a);switch(b){case "int":return a.toPlainString(null,!1).split(".")[0];case "dec":return a.toPlainString(null,!1).split(".")[1];case "sym":return a.asset.symbol;default:return Asset.parse(a).toFriendlyString(null,!0)}},openPost:function(a,b){cancelEventPropagation(b);creaEvents.emit("navigation.post.data",a,this.state,"","profile");showModal("#modal-post")},showProfile:showProfile,getJoinDate:function(){return this.lang.PROFILE.JOINED+moment((new Date(this.state.user.created)).getTime(),
"x").format("MMMM YYYY")},getBuzzClass:function(a){var k={};k[a.buzz.level_name]=!0;return k},getFeaturedImage:function(a){return(a=a.metadata.featuredImage)&&a.hash?{url:apiOptions.ipfs+a.hash}:a&&a.url?a:{}},getTags:function(a){a=a.metadata.tags;var b=[];a=a.slice(0,7);a.forEach(function(a){b.push('<a href="/search?page=1&query='+encodeURIComponent(a)+'">'+a+"</a>")});return b.join(", ")},getLinkedTags:function(a,b){var k=[];return a&&(a.forEach(function(a){k.push('<a href="/search?page=1&query='+
encodeURIComponent(a)+'">'+a+"</a>")}),b)?k.join(", "):k},hasPaid:function(a){var b=new Date;a=toLocaleDate(a.cashout_time);return b.getTime()>a.getTime()},getPayoutPostDate:function(a){var b=toLocaleDate(a.cashout_time);this.hasPaid(a)&&(b=toLocaleDate(a.last_payout));return moment(b).fromNow()},hasPromotion:function(a){return 0<Asset.parseString(a.promoted).amount},getPromotion:function(a){return"$ "+Asset.parseString(a.promoted).toPlainString()},parseJSON:function(a){if(a&&0<a.length)try{return JSON.parse(a)}catch(I){catchError(I)}return{}},
...
Asset.parse({amount:d.toFloat()*a,nai:"cbd"});d=Asset.parse({amount:NaNOr((d.toFloat()-a.toFloat())/b.toFloat(),0),nai:"cgy"});c=Asset.parse({amount:NaNOr(a.toFloat()*(c/1E4),0),nai:"cbd"});b=Asset.parse({amount:NaNOr((a.toFloat()-c.toFloat())/b.toFloat(),0),nai:"crea"});return"("+c.toFriendlyString(null,!1)+", "+b.toFriendlyString(null,!1)+", "+d.toFriendlyString(null,!1)},dateFromNow:function(a){return moment(toLocaleDate(a)).fromNow()},getFutureDate:function(a){return moment(toLocaleDate(a)).fromNow()},
...
b){if(a=Session.getAlive())a.logout(),goTo("/")}))}):b(a.lang.CHANGE_PASSWORD.ERROR_CONDITIONS):b(a.lang.CHANGE_PASSWORD.ERROR_MATCHED_PASSWORDS):b(a.lang.CHANGE_PASSWORD.ERROR_CURRENT_PASSWORD)}}});l.$forceUpdate();creaEvents.emit("crea.dom.ready")}function m(a,b,c){var f=a.replace("s","").replace("-","_"),d={rewards24Vests:0,rewardsWeekVests:0,totalRewardsVests:0,rewards24Crea:0,rewardsWeekCrea:0,totalRewardsCrea:0,rewards24CBD:0,rewardsWeekCBD:0,totalRewardsCBD:0},h=new Date,D=(new Date(h.getTime()-
864E5)).getTime(),e=(new Date(h.getTime()-6048E5)).getTime(),q,k,g=[];c.user.transfer_history.sort(function(a,b){a=Date.fromUTCString(a[1].timestamp);return Date.fromUTCString(b[1].timestamp).getTime()-a.getTime()});c.user.transfer_history.forEach(function(a){if("author_reward"===a[1].op[0]&&"author_reward"===f){k||(k=(new Date(a[1].timestamp)).getTime());q=(new Date(a[1].timestamp)).getTime();var b=Asset.parseString(a[1].op[1].vesting_payout),c=Asset.parseString(a[1].op[1].crea_payout),h=Asset.parseString(a[1].op[1].cbd_payout);
q>e&&(q>D&&(d.rewards24Vests+=b.toFloat(),d.rewards24Crea+=c.toFloat(),d.rewards24CBD+=h.toFloat()),d.rewardsWeekVests+=b.toFloat(),d.rewardsWeekCrea+=c.toFloat(),d.rewardsWeekCBD+=h.toFloat());d.totalRewardsVests+=b.toFloat();d.totalRewardsCrea+=c.toFloat();d.totalRewardsCBD+=h.toFloat();g.push(a)}else"curation_reward"===a[1].op[0]&&"curation_reward"===f&&(k||(k=(new Date(a[1].timestamp)).getTime()),q=(new Date(a[1].timestamp)).getTime(),b=Asset.parseString(a[1].op[1].reward),q>e&&(q>D&&(d.rewards24Vests+=
...
e){Object.keys(d.accounts).forEach(function(a){d.accounts[a]=parseAccount(d.accounts[a])});d.accounts[c]&&(d.user=d.accounts[c],crea.formatter.estimateAccountValue(d.user).then(function(a){d.user.estimate_account_value=a}));e.sort(function(a,b){a=Date.fromUTCString(a.created);return Date.fromUTCString(b.created).getTime()-a.getTime()});for(f=0;f<e.length;f++){var h=e[f],g=h.author+"/"+h.permlink;d.content[g]=h;d.discussion_idx[""].profile.push(g)}P(d,a,b,c)}))})}function K(a){a&&a.preventDefault();
var b=l.state.user.reward_crea_balance,c=l.state.user.reward_cbd_balance,e=l.state.user.reward_vesting_balance;requireRoleKey(l.session.account.username,"posting",function(a){globalLoading.show=!0;crea.broadcast.claimRewardBalance(a,l.session.account.username,b,c,e,function(a,b){globalLoading.show=!1;catchError(a)||updateUserSession()})})}function J(a,b,c,e,d,h,g){"function"===typeof h&&(g=h,h="");if(c)switch(c=c.account.username,b){case CONSTANTS.TRANSFER.TRANSFER_CREA:case CONSTANTS.TRANSFER.TRANSFER_CBD:crea.broadcast.transfer(a,
c,e,d,h,g);break;case CONSTANTS.TRANSFER.TRANSFER_TO_SAVINGS_CREA:case CONSTANTS.TRANSFER.TRANSFER_TO_SAVINGS_CBD:crea.broadcast.transferToSavings(a,c,e,d,h,g);break;case CONSTANTS.TRANSFER.TRANSFER_FROM_SAVINGS_CREA:case CONSTANTS.TRANSFER.TRANSFER_FROM_SAVINGS_CBD:crea.broadcast.transferFromSavings(a,c,parseInt((new Date).getTime()/1E3),e,d,h,g);break;case CONSTANTS.TRANSFER.TRANSFER_TO_VESTS:crea.broadcast.transferToVesting(a,c,e,d,g)}else g&&g(Errors.USER_NOT_LOGGED)}function H(a,b){var c=getPathPart(null,
1);c="settings"===c.toLowerCase();if(a){if(c&&(c=getPathPart(),"@"+a.account.username!==c)){showProfile(c);return}b.user.cgy_balance="0.000 "+apiOptions.symbol.CGY;a.account.username===b.user.name&&(B={op:"transfer_crea",title:lang.WALLET.TRANSFER_CREA_TITLE,text:lang.WALLET.TRANSFER_CREA_TEXT,button:lang.BUTTON.SEND,total_amount:Asset.parseString("0.000 CREA").toFriendlyString(),nai:apiOptions.symbol.CREA,confirmed:!1,to:null,disableTo:!1})}else if(c){c=getPathPart();showProfile(c);return}S(a,b)}
...
{Authorization:"Bearer "+a};b.post({author:getPathPart().replace("@",""),page:u})})}var l,v={},z,w,x,t,A,B,u=1;creaEvents.on("crea.session.login",H);creaEvents.on("crea.session.update",function(a,b){--u;H(a,b)});var C;creaEvents.on("crea.scroll.bottom",function(){C||(C=!0,G(function(a,b){b.sort(function(a,b){a=Date.fromUTCString(a.created);return Date.fromUTCString(b.created).getTime()-a.getTime()});for(a=0;a<b.length;a++){var c=b[a],e=c.author+"/"+c.permlink;l.state.content[e]=c;l.state.discussion_idx[""].profile.push(e)}l.state.discussions=
