...
!0,refreshAccessToken(function(b){(new HttpClient(apiOptions.apiUrl+("/validation/"+c))).setHeaders({Authorization:"Bearer "+b}).get().when("done",function(a){globalLoading.show=!1;a=JSON.parse(a);console.log("SignUp",a);welcomeVue.username=a.data.username;welcomeVue.slide=5;welcomeVue.sentSMS=null!==a.data.phoneToken&&void 0!==a.data.phoneToken}).when("fail",function(a,b,c){console.error(a,b,c);goTo("/"+a.status)})})):welcomeVue.slide=1})})();
